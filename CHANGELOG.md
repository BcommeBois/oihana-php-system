...
- Controllers
  ...
  - `ArchiveTrait::extractTar()` and `ArchiveTrait::extractZip()` extracting incoming archives back to disk. `extractTar()` delegates to `oihana\files\archive\tar\untar()` (already path-traversal/bomb-safe). `extractZip()` uses `ZipArchive` with home-grown guards — a **Zip Slip** check rejecting any entry that resolves outside the destination (via `oihana\files\path\canonicalizePath()` + `isBasePath()`) and decompression-bomb limits — backed by the new `ExtractOption` enum (`maxEntries`, `maxSize`, `overwrite`); it returns the list of extracted entries.
