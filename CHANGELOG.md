...
- Controllers
  - `CsrfTrait` (`oihana\controllers\traits`) exposing the CSRF token to controllers and templates through a DI-provided `Slim\Csrf\Guard` (resolved from `$init[CsrfTrait::CSRF]` or the container, never instantiated in the trait, like `HttpCacheTrait`/`FileEncryptionTrait`): `initializeCsrf()`, the read accessors `csrfTokenName()`/`csrfTokenNameKey()`/`csrfTokenValue()`/`csrfTokenValueKey()`, the aggregate `csrfTokens()` (`[ nameKey => name, valueKey => value ]` for template injection), `generateCsrfToken()` (fresh pair for a form rendered without the guard middleware) and `validateCsrf()` (manual verification). Every accessor degrades gracefully (`null` / `[]` / `false`) when the guard is not configured.
  - `ConditionalRequestTrait` (`oihana\controllers\traits`) serving a file with HTTP conditional-request support: `conditionalFileResponse()` always emits an `ETag` and a `Last-Modified` header, then evaluates the request preconditions (`If-None-Match` taking precedence over `If-Modified-Since`, RFC 7232) and returns a bodyless `304 Not Modified` when the resource is unchanged, otherwise streams the full file (`200`). Backed by the new stateless `computeETag()` helper (metadata `mtime`-`size` by default, `md5_file()` content hash on demand, strong or weak) and `etagMatches()` helper (parses the `If-None-Match` list and `*`, weak comparison), plus the `ConditionalRequestOption` enum (`weak`, `hashContent`).
  - `RangeTrait` (`oihana\controllers\traits`) serving a file with HTTP byte-range support: `rangeFileResponse()` advertises `Accept-Ranges: bytes`, streams the full file (`200`) when there is no `Range`, returns the requested slice (`206 Partial Content` + `Content-Range`) for a satisfiable single range, and `416 Range Not Satisfiable` otherwise. Multi-range requests fall back to full content. Backed by the new stateless `parseRangeHeader()` helper (handles `bytes=a-b`, `bytes=a-`, `bytes=-n`).
  - `applyContentHeaders()` helper (`oihana\controllers\helpers`) applying the download content headers (`Content-Type` / `Content-Length` / `Content-Disposition`) to a PSR-7 response from `FileResponseOption` switches — the single source of truth now shared by `FileTrait::fileResponse()`, `ArchiveTrait` and `FileEncryptionTrait`.
  - `ArchiveTrait` (`oihana\controllers\traits`) bundling files into a download response: `tarResponse()` (tar GZIP / BZIP2 / NONE, via `oihana\files\archive\tar\tar()`) and `zipResponse()` (renamed from `FileTrait::zip()`, building the archive via `oihana\files\archive\zip\zip()` with `$path` as the preserved root), sharing a private `archiveDownload()` helper for header emission, streaming and temp-file cleanup. Content headers are toggled through `FileResponseOption`. (CI now enables the `bz2` extension so bzip2 archives are exercised.)
  - `ArchiveTrait::extractTar()` and `ArchiveTrait::extractZip()` extracting incoming archives back to disk, thin wrappers delegating respectively to `oihana\files\archive\tar\untar()` and `oihana\files\archive\zip\unzip()` (both path-traversal/Zip-Slip- and decompression-bomb-safe, driven by the `TarOption`/`ZipOption` flags `maxEntries`, `maxSize`, `overwrite`, `dryRun`, `keepPermissions`). Each returns `true` on success, or the list of entries when `dryRun` is enabled.
...
- Controllers
  - `FileTrait::fileResponse()`, `ArchiveTrait` and `FileEncryptionTrait` now build their content headers through the shared `applyContentHeaders()` helper instead of duplicating the `Content-Type`/`Content-Length`/`Content-Disposition` toggle block. Side effect: `fileResponse()` no longer emits a `null` `Content-Disposition` when the header is enabled without a value — it falls back to `attachment; filename=<basename>`.
  - `FileTrait::fileResponse()` and `ImageTrait::imageResponse()` now stream the file with `StreamFactory::createStreamFromFile()` (`withBody()`) instead of loading it entirely into memory with `file_get_contents()` — the body is read lazily at emit time, so large files no longer inflate memory. Output is unchanged for clients. (Archive/encrypted responses keep buffering, as their temporary file must be removed right after.)
  ...
  - `FileTrait::zip()` was extracted into `ArchiveTrait` and renamed `zipResponse()` (see _Added_). It now accepts an `$options` array (`FileResponseOption`) and shares the `archiveDownload()` helper with `tarResponse()`; the `Content-Disposition` filename is the archive **basename** (previously the full path). It removes the temporary archive after streaming and uses `CacheControlDirective::NO_CACHE` for `Pragma`.
